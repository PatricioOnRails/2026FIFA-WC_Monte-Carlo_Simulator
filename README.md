...
| `outputs/match_predictions.csv` | One row per official match with the pre-match model probabilities (1X2, expected goals, modal score, and modal-score probability) captured immediately before the match is resolved. This file is intended for post-hoc evaluation, dashboards, and comparison of model predictions versus actual match outcomes. |
